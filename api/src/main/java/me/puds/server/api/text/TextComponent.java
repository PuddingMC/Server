...
public class TextComponent {
    ...
    public String toAnsiString() {
        StringBuilder ansiString = new StringBuilder(color.toAnsiString());
        for (Map.Entry<Boolean, TextColor> entry : List.of(
                new AbstractMap.SimpleEntry<>(bolded, TextColor.BOLD),
                new AbstractMap.SimpleEntry<>(bolded, TextColor.BOLD),
                new AbstractMap.SimpleEntry<>(italicized, TextColor.ITALIC),
                new AbstractMap.SimpleEntry<>(underlined, TextColor.UNDERLINE),
                new AbstractMap.SimpleEntry<>(struckthrough, TextColor.STRIKETHROUGH)
        )) {
            if (entry.getKey()) {
                ansiString.append(entry.getValue().toAnsiString());
            }
        }
        ansiString.append(text);

        for (TextComponent extra : extras) {
            ansiString.append(extra.toAnsiString());
        }
