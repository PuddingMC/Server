...
public enum Biome {
    ...
    Biome(int numericalId) {
        this.numericalId = numericalId;
    }

    public NamespacedKey toIdentifier() {
        return NamespacedKey.minecraft(name().toLowerCase());
    }

    public static Biome fromNumericalId(int id) {
        return Arrays.stream(values())
                .filter(b -> b.getNumericalId() == id)
                .findFirst()
                .orElse(values()[0]);
    }

    public static Biome fromIdentifier(NamespacedKey identifier) {
        String value = identifier.getKey().toUpperCase();
